...
const App = () => {
  ...
  const skillBadge = (items) => {
    return items.map((item, i) => (
      <Badge
        key={i}
        variant={"outline"}
        className="border border-zinc-700 text-gray-400 text-xs sm:text-sm px-2 py-1"
      >
        {item}
      </Badge>
    ));
  ...
  return (
    <div className="">
      ...
      />
      <main
        className={`flex min-h-screen flex-col text-white p-8 md:p-16 lg:p-24 max-w-3xl mx-auto z-100 relative`}
      >
        <div className="mt-10 md:mt-0 mb-10 flex sm:flex-row items-center gap-5 rounded-md justify-between">
          <div className="flex flex-row gap-5 items-center">
            <div className="overflow-hidden flex items-center justify-center w-16 sm:w-20 rounded-full">
              <img
                src="/pfp_x_ghibli.png"
                alt=""
                className="object-cover w-full h-full"
              />
            </div>
            <div className="scale-95 flex flex-col">
              <span className="font-sans font-semibold text-2xl sm:text-4xl">
                Saksham
              </span>
              <span className="font-mono text-zinc-500 leading-relaxed tracking-normal text-sm sm:text-base">
                @skshmgpt
              </span>
            </div>
          </div>
        ...
        <section id="about">
          <div className="text-gray-400 leading-relaxed tracking-wide font-mono text-sm">
            ...
            <p className="mb-3 text-white mt-2">doing cool stuff</p>
          </div>
        </section>
        ...
        <section>
          <div className="text-gray-400 leading-relaxed tracking-wide font-mono text-xs sm:text-sm">
            <h1 className="font-bold text-xl sm:text-2xl mb-4 text-white">
              Skills
            </h1>
            {Object.entries(data["skills"]).map(([category, items]) => (
              <div key={category} className="mb-4 text-white">
                <span className="text-white mb-2 mr-3 block sm:inline text-sm sm:text-base">
                  {category}
                </span>
